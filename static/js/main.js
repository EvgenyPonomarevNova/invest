document.addEventListener('DOMContentLoaded', () => {
  ...
  if (burgerBtn.length > 0) {
    burgerBtn.forEach(function (toggleBtn) {
      toggleBtn.addEventListener('click', function (e) {
        ...
        document.querySelector('.header__content').classList.toggle('header__content_show');
      });
    });
  } // Anchor
  ...
  function mask(event) {
    ...
    let reg = new RegExp('^' + matrix.substr(0, this.value.length).replace(/_+/g, function (a) {
      return '\\d{1,' + a.length + '}';
    }).replace(/[+()]/g, '\\$&') + '$');
  ...
  function activateTab(index) {
    tabs.forEach(tab => tab.classList.remove('steps-section__tabs-item_active'));
    contents.forEach(content => content.classList.remove('steps-section__content_show'));
    if (tabs[index]) tabs[index].classList.add('steps-section__tabs-item_active');
    if (contents[index]) contents[index].classList.add('steps-section__content_show');
    currentIndex = index;
  }
...
if (heroVideo) {
  ...
  function animateCount(el) {
    ...
    function update(currentTime) {
      ...
      el.textContent = end % 1 !== 0 ? currentValue.toFixed(1) : Math.floor(currentValue);
